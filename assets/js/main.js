...
(function ($) {
	...
	////////////////////////////////////////////////////
	// 03. Offcanvas Js
	$(".rs_header-bars").on("click", function () {
		$(".rs_offcanvas-area").addClass("rs_offcanvas-open");
		// $(".offcanvas__full").addClass("offcanvas-full-opened");
		$(".rs_body-overlay").addClass("opened");
	});

	$(".rs_offcanvas-close-btn").on("click", function () {
		$(".rs_offcanvas-area").removeClass("rs_offcanvas-open");
		// $(".offcanvas__full").removeClass("offcanvas-full-opened");
		$(".rs_body-overlay").removeClass("opened");
	});

	////////////////////////////////////////////////////
	//  Body overlay Js
	$(".rs_body-overlay").on("click", function () {
		$(".rs_offcanvas-area").removeClass("rs_offcanvas-open");
		// $(".offcanvas__full").removeClass("offcanvas-full-opened");
	...
	var swiper = new Swiper('.filter_slider', {
		...
		spaceBetween: 30,
		navigation: {
			nextEl: '.rs_filter-next',
			prevEl: '.rs_filter-prev',
		},
		watchOverflow: false,
		breakpoints: {
	...
	document.querySelectorAll('.rs_filter-tabs button').forEach(function (button) {
		button.addEventListener('click', function () {
			// Remove active class from all buttons
			document.querySelectorAll('.rs_filter-tabs button').forEach(function (btn) {
				btn.classList.remove('rs_filter-tab-active');
			});
			// Add active class to the clicked button
			this.classList.add('rs_filter-tab-active');
			// Get the filter value and split it by space (for multiple categories)
			var filterValues = this.getAttribute('data-filter').split(' ').map(item => item.trim());

			// Loop through all slides and show/hide based on filter
			document.querySelectorAll('.filter_slider .swiper-slide').forEach(function (slide) {
				// Check if the slide matches any of the categories
				var matchesFilter = filterValues.some(function (filterValue) {
					return filterValue === '*' || slide.classList.contains(filterValue.replace('.', ''));
				});

				// If the slide matches any category, show it, otherwise hide it
				if (matchesFilter) {
					slide.classList.remove('hidden-item'); // Show matching items
				} else {
					slide.classList.add('hidden-item'); // Hide non-matching items
				}
			});

			// Update Swiper after showing/hiding slides to recalculate the layout
			swiper.update();
		});
	});
	...
	// testi slider 
	var swiper = new Swiper('.rs_testimonial-slider', {
		slidesPerView: 1,
		spaceBetween: 30,
		loop: true,
		effect: 'fade',
		fadeEffect: {
			crossFade: true, // Ensures smoother transitions without overlap
		},
		speed: 300,
		navigation: {
			nextEl: '.rs_testi-next',
			prevEl: '.rs_testi-prev ',
		},
		autoplay: {
			delay: 3000,
			disableOnInteraction: false,
